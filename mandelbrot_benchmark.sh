...
NUMBER=11 # Example depth, adjust as needed
...
for i in {1..1}; do
    # For C
    time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/mandelbrot/mandelbrot.c -o ${BENCH_DIR}/mandelbrot/mandelbrot_c && ${BENCH_DIR}/mandelbrot/mandelbrot_c ${NUMBER}"
    
    # For C++
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/mandelbrot/mandelbrot.cpp -o ${BENCH_DIR}/mandelbrot/mandelbrot_cpp && ${BENCH_DIR}/mandelbrot/mandelbrot_cpp ${NUMBER}"
    
    # For Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/mandelbrot/mandelbrot.py ${NUMBER}"
    
    # For Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/mandelbrot/mandelbrot_codon.py ${NUMBER}"
    
    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/mandelbrot/mandelbrot_c" "${BENCH_DIR}/mandelbrot/mandelbrot_cpp"
done

...
do
    ...
    ${CPP} -std=c++17 -O0 "${BENCH_DIR}/taq.cpp" -o "${BENCH_DIR}/taq_cpp_o0"  
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/taq_cpp_o0" ${SIZE})
    # echo "$CPP_OUTPUT"
    ...
    ${CPP} -std=c++17 -O3 "${BENCH_DIR}/taq.cpp" -o "${BENCH_DIR}/taq_cpp_o3"
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/taq_cpp_o3" ${SIZE})
    # echo "$CPP_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    rm "${BENCH_DIR}/taq_cpp_o0"
    rm "${BENCH_DIR}/taq_cpp_o3"
    rm "${BENCH_DIR}/taq"

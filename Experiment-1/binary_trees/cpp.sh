...
log_process_stats() {
    ...
    sleep 0.01  # Short delay to ensure PowerJoular starts monitoring
    ...
    sudo kill -INT $powerjoular_pid
    sleep 1  # Ensure PowerJoular finalizes the data writing

    local cpu_avg=$(awk -F',' '{cpu+=$1} END {print cpu/NR}' "$stats_file")
    local mem_avg=$(awk '{mem+=$2} END {print mem/NR}' "$stats_file")
    local power_avg=$(awk -F',' 'NR > 1 {power+=$3} END {print power/(NR-1)}' "${BENCH_DIR}/power-cpp-${CPP_PID}.csv")
    local execution_time=$(echo "$(${PYTHON} -c "import time; print(time.time())") - $start_time" | bc)

    echo "$execution_time,$cpu_avg,$mem_avg,$power_avg"
}
...
"${BENCH_DIR}/binary_trees_cpp" ${SIZE} 1> /dev/null &
CPP_PID=$!
CPP_STATS=$(log_process_stats $CPP_PID $START_TIME)
IFS=',' read execution_time cpu_usage mem_usage power_avg <<< "$CPP_STATS"

...
for i in {1..1}; do
    # For C
    time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/nbody/nbody.c -o ${BENCH_DIR}/nbody/nbody_c && ${BENCH_DIR}/nbody/nbody_c ${NUMBER}"
    
    # For C++
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/nbody/nbody.cpp -o ${BENCH_DIR}/nbody/nbody_cpp && ${BENCH_DIR}/nbody/nbody_cpp ${NUMBER}"
    
    # For Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/nbody/nbody.py ${NUMBER}"
    
    # For Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/nbody/nbody.py ${NUMBER}"
    
    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/nbody/nbody_c" "${BENCH_DIR}/nbody/nbody_cpp"
done

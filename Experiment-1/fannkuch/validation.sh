...
analyze_complexity "${BENCH_DIR}/fannkuch.cpp" "C++"
analyze_complexity "${BENCH_DIR}/fannkuch.py" "Python"
analyze_complexity "${BENCH_DIR}/fannkuch.py" "Codon"
...
do
    ...
    ${CPP} -std=c++17 -O0 "${BENCH_DIR}/fannkuch.cpp" -o "${BENCH_DIR}/fannkuch_cpp_o0"  
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/fannkuch_cpp_o0" ${SIZE})
    # echo "$CPP_OUTPUT"
    ...
    ${CPP} -std=c++17 -O3 "${BENCH_DIR}/fannkuch.cpp" -o "${BENCH_DIR}/fannkuch_cpp_o3"
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/fannkuch_cpp_o3" ${SIZE})
    # echo "$CPP_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    PYTHON_OUTPUT=$(${PYTHON} "${BENCH_DIR}/fannkuch.py" ${SIZE})
    # echo "$PYTHON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CODON_OUTPUT=$("${BENCH_DIR}/fannkuch" ${SIZE})
    # echo "$CODON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    rm "${BENCH_DIR}/fannkuch_cpp_o0"
    rm "${BENCH_DIR}/fannkuch_cpp_o3"

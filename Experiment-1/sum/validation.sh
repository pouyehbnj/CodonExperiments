...
analyze_complexity "${BENCH_DIR}/sum.cpp" "C++"
analyze_complexity "${BENCH_DIR}/sum.py" "Python"
analyze_complexity "${BENCH_DIR}/sum.py" "Codon"
...
do
    ...
    ${CPP} -std=c++17 -O0 "${BENCH_DIR}/sum.cpp" -o "${BENCH_DIR}/sum_cpp_o0"  
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/sum_cpp_o0" ${SIZE})
    # echo "$CPP_OUTPUT"
    ...
    ${CPP} -std=c++17 -O3 "${BENCH_DIR}/sum.cpp" -o "${BENCH_DIR}/sum_cpp_o3"
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/sum_cpp_o3" ${SIZE})
    # echo "$CPP_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    PYTHON_OUTPUT=$(${PYTHON} "${BENCH_DIR}/sum.py" ${SIZE})
    # echo "$PYTHON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    ${CODON} build --release "${BENCH_DIR}/sum.py"  
    
    # Run Codon Python program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CODON_OUTPUT=$("${BENCH_DIR}/sum" ${SIZE})
    # echo "$CODON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    rm "${BENCH_DIR}/sum_cpp_o0"
    rm "${BENCH_DIR}/sum_cpp_o3"
    rm "${BENCH_DIR}/sum"

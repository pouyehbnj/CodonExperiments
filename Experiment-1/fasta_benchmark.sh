...
benchmark_name="fasta"
results_file="benchmark_results.csv"
SIZE=25000000  # Example depth, adjust as needed
...
for i in {1..1}; do
    # For C
    time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/fasta/fasta.c -o ${BENCH_DIR}/fasta/fasta_c && ${BENCH_DIR}/fasta/fasta_c ${SIZE}"
    
    # For C++
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/fasta/fasta.cpp -o ${BENCH_DIR}/fasta/fasta_cpp && ${BENCH_DIR}/fasta/fasta_cpp ${SIZE}"
    
    # For Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/fasta/fasta.py ${SIZE}"
    
    # For Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/fasta/fasta.py ${SIZE}"
    
    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/fasta/fasta_c" "${BENCH_DIR}/fasta/fasta_cpp"
done

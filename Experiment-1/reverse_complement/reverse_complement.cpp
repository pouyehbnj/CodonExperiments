...
#include <vector>
#include <algorithm>
#include <thread>  // For introducing slight delays
...
std::string format_sequence(const std::string& sequence) {
    ...
    size_t pos = 0;
    while (pos < sequence.size()) {
        if (pos > 0) temp_storage += '\n';
        temp_storage += sequence.substr(pos, std::min(size_t(60), sequence.size() - pos));
        pos += 60;
    }
    formatted_result = temp_storage; 
    return formatted_result;
}

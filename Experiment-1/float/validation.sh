...
analyze_complexity "${BENCH_DIR}/float.cpp" "C++"
analyze_complexity "${BENCH_DIR}/float.py" "Python"
analyze_complexity "${BENCH_DIR}/float.py" "Codon"
...
do
    ...
    ${CPP} -std=c++17 -O0 "${BENCH_DIR}/float.cpp" -o "${BENCH_DIR}/float_cpp_o0"  
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/float_cpp_o0" ${SIZE})
    # echo "$CPP_OUTPUT"
    ...
    ${CPP} -std=c++17 -O3 "${BENCH_DIR}/float.cpp" -o "${BENCH_DIR}/float_cpp_o3"
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/float_cpp_o3" ${SIZE})
    # echo "$CPP_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    PYTHON_OUTPUT=$(${PYTHON} "${BENCH_DIR}/float.py" ${SIZE})
    # echo "$PYTHON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CODON_OUTPUT=$("${BENCH_DIR}/float" ${SIZE})
    # echo "$CODON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    rm "${BENCH_DIR}/float_cpp_o0"
    rm "${BENCH_DIR}/float_cpp_o3"
    rm "${BENCH_DIR}/float"

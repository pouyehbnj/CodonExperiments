...
for i in {1..1}; do
    # For C
    time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/fannkuch/fannkuch.c -o ${BENCH_DIR}/fannkuch/fannkuch_c && ${BENCH_DIR}/fannkuch/fannkuch_c ${NUMBER}"
    
    # For C++
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/fannkuch/fannkuch.cpp -o ${BENCH_DIR}/fannkuch/fannkuch_cpp && ${BENCH_DIR}/fannkuch/fannkuch_cpp ${NUMBER}"
    
    # For Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/fannkuch/fannkuch.py ${NUMBER}"
    
    # For Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/fannkuch/fannkuch_codon.py ${NUMBER}"
    
    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/fannkuch/fannkuch_c" "${BENCH_DIR}/fannkuch/fannkuch_cpp"
done

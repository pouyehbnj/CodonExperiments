...
for i in {1..1}; do
    # For C
    # time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/taq/taq.c -o ${BENCH_DIR}/taq/taq_c && ${BENCH_DIR}/taq/taq_c ${LIMIT}"
    
    # For C++
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/taq/taq.cpp -o ${BENCH_DIR}/taq/taq_cpp && ${BENCH_DIR}/taq/taq_cpp ${SIZE}"
    
    # For Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/taq/taq.py ${SIZE}"
    
    # For Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/taq/taq.py ${SIZE}"
    
    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/taq/taq_cpp"
done

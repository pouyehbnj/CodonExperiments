...
analyze_complexity "${BENCH_DIR}/word_count.cpp" "C++"
analyze_complexity "${BENCH_DIR}/word_count.py" "Python"
analyze_complexity "${BENCH_DIR}/word_count.py" "Codon"
...
${PYTHON} "${BENCH_DIR}/text_generator.py" "${SIZE}"
...
do
    ...
    # Compile C++ program
    ${CPP} -std=c++17 -O0 "${BENCH_DIR}/word_count.cpp" -o "${BENCH_DIR}/word_count_cpp_o0"
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/word_count_cpp_o0")
    # echo "$CPP_OUTPUT"
    ...
    echo "${i},cpp,${CPP_TIME_O0},o0,${SIZE}" >> "${CSV_FILE}"
     ...
     # Compile C++ program
    ${CPP} -std=c++17 -O3 "${BENCH_DIR}/word_count.cpp" -o "${BENCH_DIR}/word_count_cpp_o3"
    
    # Run C++ program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CPP_OUTPUT=$("${BENCH_DIR}/word_count_cpp_o3")
    # echo "$CPP_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    PYTHON_OUTPUT=$(${PYTHON} "${BENCH_DIR}/word_count.py")
    # echo "$PYTHON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    ${CODON} build --release -numerics=py "${BENCH_DIR}/word_count.py"
    
    # Run Codon Python program and measure time
    START_TIME=$(${PYTHON} -c "import time; print(time.time())")
    CODON_OUTPUT=$("${BENCH_DIR}/word_count")
    # echo "$CODON_OUTPUT"
    END_TIME=$(${PYTHON} -c "import time; print(time.time())")
    ...
    rm "${BENCH_DIR}/word_count_cpp_o0"
    rm "${BENCH_DIR}/word_count_cpp_o3"
    rm  "${BENCH_DIR}/word_count"

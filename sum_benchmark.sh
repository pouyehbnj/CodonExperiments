...
for i in {1..50}; do
    time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/sum/sum.c -o ${BENCH_DIR}/sum/sum_c && ${BENCH_DIR}/sum/sum_c ${NUMBERS}"

    # C++ (compile and run as a single step for timing)
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/sum/sum.cpp -o ${BENCH_DIR}/sum/sum_cpp && ${BENCH_DIR}/sum/sum_cpp ${NUMBERS}"

    # Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/sum/sum.py ${NUMBERS}"

    # Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/sum/sum.py ${NUMBERS}"

    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/sum/sum_c" "${BENCH_DIR}/sum/sum_cpp"
    done

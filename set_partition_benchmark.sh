...
## # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #  
benchmark_name="set_partition"
# C (compile and run as a single step for timing)
for i in {1..1}; do
    # time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/set_partition/set_partition.c -o ${BENCH_DIR}/set_partition/set_partition_c && ${BENCH_DIR}/set_partition/set_partition_c ${SIZE}"

    # C++ (compile and run as a single step for timing)
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/set_partition/set_partition.cpp -o ${BENCH_DIR}/set_partition/set_partition_cpp && ${BENCH_DIR}/set_partition/set_partition_cpp ${SIZE}"

    # Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/set_partition/set_partition.py ${SIZE}"

    # Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/set_partition/set_partition.py ${SIZE}"

    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/set_partition/set_partition_cpp"
    done

echo  "${benchmark_name} benchmarkcompleted."
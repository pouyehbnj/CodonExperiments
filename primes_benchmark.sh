...
for i in {1..1}; do
    # For C
    time_execution "C" "${C} -std=c11 -O3 ${BENCH_DIR}/primes/primes.c -o ${BENCH_DIR}/primes/primes_c && ${BENCH_DIR}/primes/primes_c ${LIMIT}"
    
    # For C++
    time_execution "C++" "${CPP} -std=c++17 -O3 ${BENCH_DIR}/primes/primes.cpp -o ${BENCH_DIR}/primes/primes_cpp && ${BENCH_DIR}/primes/primes_cpp ${LIMIT}"
    
    # For Python
    time_execution "Python" "${PYTHON} ${BENCH_DIR}/primes/primes.py ${LIMIT}"
    
    # For Codon
    time_execution "Codon" "${CODON} run -release ${BENCH_DIR}/primes/primes.py ${LIMIT}"
    
    # Clean up
    rm $time_output
    rm "${BENCH_DIR}/primes/primes_c" "${BENCH_DIR}/primes/primes_cpp"
done

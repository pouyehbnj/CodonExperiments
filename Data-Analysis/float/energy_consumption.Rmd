...
# Function to clean and convert CPU Power data
clean_and_convert_cpu_power <- function(cpu_power) {
  # Replace non-numeric characters and convert to numeric
  cleaned_power <- gsub("[^0-9\\.]", 0, cpu_power)  # Remove anything that's not a digit or decimal point
  as.numeric(cleaned_power)  # Convert to numeric, non-numeric becomes NA
}
    ...
    if (file.exists(power_file_name)) {
      power_data <- read_csv(power_file_name, show_col_types = FALSE) %>%
        mutate(
          ...
          `CPU Power` = clean_and_convert_cpu_power(`CPU Power`)
        ) %>%

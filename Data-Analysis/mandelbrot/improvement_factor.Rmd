...
process_improvements <- function(codon_data, other_data, comparison_name) {
  ...
  # Ensure valid data points are used to calculate medians
  improvements <- bind_rows(lapply(metrics, function(metric) {
    median_codon <- codon_data %>%
      group_by(SIZE_CATEGORY) %>%
      summarize(Median_Codon = ifelse(all(is.na(!!sym(metric))), NA, median(!!sym(metric), na.rm = TRUE)), .groups = 'drop')
    
    median_other <- other_data %>%
      group_by(SIZE_CATEGORY) %>%
      summarize(Median_Other = ifelse(all(is.na(!!sym(metric))), NA, median(!!sym(metric), na.rm = TRUE)), .groups = 'drop')

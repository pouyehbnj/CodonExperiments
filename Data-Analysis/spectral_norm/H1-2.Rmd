...
create_density_plot <- function(group1, group2, group1_label, group2_label, size_category, metric) {
  ...
  # Plot setup with dynamic x-axis scaling and adjusted bandwidth
  adjust_density <- 1.25
  max_value <- max(combined_data$Values, na.rm = TRUE)
  min_value <- min(combined_data$Values, na.rm = TRUE)
  ...
  p <- ggplot(combined_data, aes(x = Values, fill = Group)) +
    geom_density(alpha = 0.5, adjust = adjust_density) +  # adjust parameter for better visualization
